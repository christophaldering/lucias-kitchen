...
- **App Shell Navigation** (`src/App.tsx`): Compact header (logo left + avatar right, no tab bar). Fixed bottom navigation with 5 icon+label tabs (Rezepte, Kochidee/Lightbulb, Wochenplan, Einladungen, Statistiken). Einladungen tab shows unread notification badge count. Active tab highlighted with amber/orange indicator. Safe-area-inset support for iPhones. Avatar dropdown has: Mein Profil, Einstellungen, Abmelden.
- **Kocheinladungen & Wunschzettel** (`src/pages/Einladungen.tsx`, `src/hooks/useInvitations.ts`): Full invitation system. Hosts create invitations with one of 4 modes (Überraschung/surprise, Wunschzettel/wishlist, Abstimmung/vote, Auswahl/choice), select members to invite, and optionally propose recipes for voting. Guests receive in-app notifications, see invitations in Wochenplan day cells (blue dot + info card), can RSVP (coming/not coming) and respond per mode (wish text, recipe vote, etc.). Host sees response summary and vote results (with bar chart), then confirms a final recipe which notifies all guests. Notifications stored in DB (`notifications` table), polled every 30s. API: GET/POST/PATCH/DELETE `/api/meal-invitations`, POST `/api/meal-invitations/:id/wishes`, PATCH `/api/meal-invitations/:id/rsvp`, GET/PATCH `/api/notifications`, GET `/api/users`.
...
- **DB schema**: `lib/db/src/schema/users.ts` — `users` table; `lib/db/src/schema/invitations.ts` — `meal_invitations`, `meal_invitation_members`, `meal_wishes`, `notifications` tables with enums for mode/status/rsvp
